...
│   └── operations.py           # Strategy + Factory + Decorator patterns — arithmetic operations
├── tests/
│   ├── test_calculation.py
│   ├── test_calculator.py
...
```bash
git clone https://github.com/denrico-njit/midterm_project.git
cd midterm_project
```
...
| `int_divide`     | Integer division, truncates toward zero              |
| `modulo`         | Remainder after division                             |
| `power`          | Raise the first number to the power of the second    |
| `root`           | Calculate the nth root of a number                   |
| `percentage`     | Calculate percentage of one number relative to another|
| `abs_difference` | Absolute difference between two numbers              |
...
### Example session
```
The Python REPL Calculator is ready to use! Type 'help' for commands.
...
`IntegerDivision`, `Modulo`, `Power`, `Percentage`, `AbsoluteDifference`, `Root`)
is encapsulated as a class implementing the abstract `Operation` interface. The
`Calculator` holds a reference to the current operation strategy, which can be
swapped at runtime without modifying the core calculation logic.
...
snapshot; redo reapplies it. This disconnects state management from calculation
logic and supports arbitrary depth up to `MAX_HISTORY_SIZE`.

### Decorator — `register_operation` (`app/operations.py`)

The `register_operation` decorator attaches help metadata (name, description,
usage example) directly to each operation. The REPL's
help menu iterates over registered operations and reads this metadata dynamically,
meaning new operations automatically appear in the help menu without any changes
to the REPL code.
...
Enforce 90% coverage (mirrors the CI check):
```bash
pytest --cov=app --cov-fail-under=90 tests/
```

Tests are organized to mirror the `app/` module structure and cover both valid
inputs and edge cases including division by zero, negative roots, invalid input
formats, configuration errors, observer behavior, undo/redo state management,
and CSV serialization round-trips.

---

## Continuous Integration

GitHub Actions runs the test suite and enforces 90% coverage on every push and
